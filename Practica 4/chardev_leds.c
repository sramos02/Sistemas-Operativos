...
static ssize_t device_write(struct file *filp, const char *buff, size_t len, loff_t * off){
	...
	char copyBuffer[BUF_LEN];
	int mask = 0;
	
   	if(len > BUF_LEN) return -ENOSPC;
  

	if(copy_from_user(copyBuffer, buff, len) == 0){
		int i;
		for(i = 0; i < len; i++){
			if(copyBuffer[i] == '1') mask |= 0x2;		
			else if(copyBuffer[i] == '2') mask |= 0x4;
	    		else if(copyBuffer[i] == '3') mask |= 0x1;
